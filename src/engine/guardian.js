...
async function manejarNav(detalles) 
{
    ...
    if (urlTarget.startsWith('chrome://') || urlTarget.startsWith('edge://') || urlTarget.startsWith(chrome.runtime.getURL('')))
        return;
    try 
    {
        ...
        let esNewTab = false;
        if (!urlSource || urlSource === '' || urlSource === 'chrome://newtab/' || urlSource === 'chrome://new-tab-page/' || urlSource === 'about:blank') 
        {
            if (tab.openerTabId) 
            {
                try 
        ...
        if (!urlSource || urlSource.startsWith('chrome://')) 
            return;
